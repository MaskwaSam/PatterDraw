...
/**
 * Adapted from Excalidraw commit 792ea3a (MIT License).
 * Kept wrapper-owned so PatterDraw can stay on the stable editor package.
 */
const isOpaqueColor = (color: unknown): boolean => {
  if (typeof color !== "string") return false;
  const normalized = color.trim().toLowerCase();
  if (!normalized) return false;
  if (isTransparent(normalized)) return false;
  if (/^#[0-9a-f]{4}$/i.test(normalized)) return normalized[4] === "f";
  if (/^#[0-9a-f]{8}$/i.test(normalized)) return normalized.slice(7) === "ff";
  const functionalAlpha = normalized.match(/^(?:rgba|hsla)\([^)]*[,/]\s*([\d.]+)(%)?\s*\)$/);
  if (!functionalAlpha) return true;
  const alpha = Number(functionalAlpha[1]);
  return Number.isFinite(alpha) && (functionalAlpha[2] ? alpha >= 100 : alpha >= 1);
};

const isTransparentColor = (color: unknown): boolean =>
  typeof color !== "string" || color.trim() === "" || isTransparent(color);

const BUCKET_GEOMETRY_ELEMENT_TYPES = new Set([
  "rectangle",
  "diamond",
  "ellipse",
  "embeddable",
...
const isFinitePoint = (value: unknown): value is readonly [number, number] =>
  Array.isArray(value)
  && value.length >= 2
...
/**
 * Imported and legacy scenes can contain partially malformed element records.
 * Bucket geometry is best-effort: preserve those records in the scene, but do
 * not let one invalid mark abort filling an otherwise valid region.
 */
export const isBucketGeometryElement = (
  value: unknown,
): value is NonDeletedExcalidrawElement => {
  if (!value || typeof value !== "object") return false;
  const element = value as Record<string, unknown>;
  if (
    typeof element.id !== "string"
    || typeof element.type !== "string"
    || !BUCKET_GEOMETRY_ELEMENT_TYPES.has(element.type)
  ...
  for (const key of ["x", "y", "width", "height", "angle", "opacity", "strokeWidth"] as const) {
    const numericValue = element[key];
  ...
  if (element.type === "line" || element.type === "arrow" || element.type === "freedraw") {
    const minimumPoints = element.type === "freedraw" ? 1 : 2;
    if (
      !Array.isArray(element.points)
      || element.points.length < minimumPoints
...
const polygonSignedAreaExact = <Point extends readonly [number, number]>(
  polygon: readonly Point[],
): number => {
  const closed = polygon.length > 1
    && polygon[0][0] === polygon[polygon.length - 1][0]
    && polygon[0][1] === polygon[polygon.length - 1][1];
  const points = closed ? polygon.slice(0, -1) : polygon;
  let sum = 0;
  for (let index = 0, previous = points.length - 1; index < points.length; previous = index, index += 1) {
    sum += points[previous][0] * points[index][1] - points[index][0] * points[previous][1];
  }
  return sum / 2;
};

const polygonAreaExact = <Point extends readonly [number, number]>(
  polygon: readonly Point[],
): number => Math.abs(polygonSignedAreaExact(polygon));

const getFreedrawStrokeCenterPoints = (
  element: ExcalidrawFreeDrawElement,
): [number, number][] =>
  getStrokePoints(element.points as unknown as number[][], {
    size: element.strokeWidth * 4.25,
    streamline: element.strokeOptions?.streamline ?? 0.5,
    last: true,
...
export type BucketFillOptions = {
  ...
  /** bail out before topology/intersection candidate checks monopolize the UI */
...
const clipSegmentToVisible = (
  ...
  eps: number,
  margin: number,
  consumeTopologyCheck: () => boolean,
): [GlobalPoint, GlobalPoint][] | null => {
  let intervals: [number, number][] = [[0, 1]];
  for (const coverer of coverers) {
    if (intervals.length === 0) {
      break;
    }
    if (!consumeTopologyCheck()) {
      return null;
    }
...
export const rendersOpaqueFill = (element: ExcalidrawElement): boolean => {
  ...
  if (
    ...
    (!Array.isArray(element.points) || !isPathALoop(element.points))
  ) {
    return false;
  }
  return true;
...
const rendersAnyMark = (element: ExcalidrawElement): boolean =>
  !isInvisible(element) &&
  (element.type === "image" ||
    !isTransparentColor(element.strokeColor) ||
    (hasBackground(element.type) && !isTransparentColor(element.backgroundColor)));

/** Wrapper-owned PDF page images are immutable canvas backgrounds, not marks
 * that bucket paint must remain beneath. */
const isClassroomPdfBackground = (element: ExcalidrawElement): boolean =>
  element.type === "image" &&
  element.locked === true &&
 ...
 * stroke. Compatibility is deliberately recognized by SHAPE so a user can
 * still restyle a directly clicked legacy/hand-drawn polygon. The app layer
 * additionally records owner/boundary metadata on newly generated fills to
 * distinguish identical regions stacked above one another; that association
 * is orthogonal to this geometry predicate and survives style changes.
 */
export const isBucketFillCompatible = (
  ...
  !isInvisible(element) &&
  !isTransparentColor(element.backgroundColor) &&
  isTransparentColor(element.strokeColor);
...
const isClosedOwnerCandidate = (element: ExcalidrawElement): boolean => {
  ...
  if (isLineElement(element)) {
    return element.polygon && Array.isArray(element.points) && isValidPolygon(element.points);
  }
  if (isFreeDrawElement(element)) {
    return Array.isArray(element.points) && isPathALoop(element.points);
  }
  return true;
};
...
const buildFaces = (
  ...
  for (let oi = 0; oi < byMinX.length; oi++) {
    ...
    for (let oj = oi + 1; oj < byMinX.length; oj++) {
      ...
      if (topologyChecks >= options.maxTopologyChecks) {
        return null;
      }
      topologyChecks += 1;
...
const selectFaceFromArrangement = (
  ...
  for (const face of faces) {
    ...
    areaOf.set(face, -polygonSignedAreaExact(face.ring));
  }
...
const spliceHoleIntoRing = (
  ...
  // export changes)
  const oriented =
    Math.sign(polygonSignedAreaExact(hole)) ===
    Math.sign(polygonSignedAreaExact(ring))
      ? [...hole].reverse()
      : hole;
...
const finalizePolygon = (
  ...
  const byAreaDesc = holeRings
    ...
    .sort((a, b) => polygonAreaExact(b.holeRing) - polygonAreaExact(a.holeRing));
...
export const computeBucketFillPolygon = (args: {
  ...
  const elements = args.elements.filter(isBucketGeometryElement);
  ...
  const collectSegments = (
    ...
    let exceededSegmentBudget = false;
    let collectionTopologyChecks = 0;
    const consumeCollectionTopologyCheck = () => {
      if (collectionTopologyChecks >= options.maxTopologyChecks) return false;
      collectionTopologyChecks += 1;
      return true;
    };
    const collect = (element: ExcalidrawElement) => {
      const elementIndex = indexOf.get(element.id) ?? 0;
      const coverersAbove: ExcalidrawElement[] = [];
      for (const coverer of coverers) {
        if (!consumeCollectionTopologyCheck()) {
          exceededSegmentBudget = true;
          return;
        }
        if (
          ...
          if (
            (isLineElement(coverer) || isFreeDrawElement(coverer))
            && coverer.points.length - 1 > options.maxBoundarySegments
          ) {
            exceededSegmentBudget = true;
            return;
          }
          coverersAbove.push(coverer);
        }
      }
      ...
      if (
        (isLineElement(element) || isFreeDrawElement(element))
        && element.points.length - 1 > options.maxBoundarySegments - rawSegments.length
      ) {
        // Reject oversized imported paths before Excalidraw expands/smooths all
        // of their points. The ordinary collection loop still catches cases
        // where curve expansion or visibility clipping produces extra pieces.
        exceededSegmentBudget = true;
        return;
      }
      ...
      for (const segment of segments) {
        ...
        const visibleSegments = clipSegmentToVisible(
          segment[0],
          segment[1],
          coverersAbove,
          elementsMap,
          options.snapEpsilon,
          clipMargin,
          consumeCollectionTopologyCheck,
        );
        if (!visibleSegments) {
          exceededSegmentBudget = true;
          return;
    ...
    for (const element of primary ? [primary, ...boundaries] : boundaries) {
      if (exceededSegmentBudget) {
        break;
      }
      collect(element);
    }
    return exceededSegmentBudget
      ? null
      : rawSegments;
  };
  ...
  if (polygonAreaExact(scenePoints) < options.minArea) {
  ...
  //   opaque fill would bury them). The overlap test is symmetric so labels
  //   and icons that only partially cross the region remain visible too. The even-odd
  //   containment matches the render rule, so elements inside a keyhole hole
  //   are correctly NOT constrained (the fill doesn't paint there).
  //
  ...
  const paintOverlapsRegion = (element: ExcalidrawElement): boolean => {
    ...
    // Exact crossing detection catches narrow marks where neither sampled
    // endpoint is contained. Bound the pair count so a pathological imported
    // free-draw cannot turn this z-order check into quadratic work.
    exactIntersections: if (outline.length * scenePoints.length <= 50_000) {
      for (const outlineSegment of outline) {
        for (let i = 0; i < scenePoints.length; i += 1) {
          if (overlapTopologyChecks >= options.maxTopologyChecks) {
            break exactIntersections;
          }
          overlapTopologyChecks += 1;
          const next = (i + 1) % scenePoints.length;
          if (lineSegmentIntersectionPoints(
            outlineSegment,
            lineSegment(scenePoints[i], scenePoints[next]),
            options.snapEpsilon,
          )) {
            return true;
          }
  ...
  for (const element of elements) {
    ...
    // participant nor an in-region mark — missing it would insert
    ...
    const mustStayAbove =
      ...
      (!isClassroomPdfBackground(element) && rendersAnyMark(element) && paintOverlapsRegion(element));
    if (mustStayAbove) {
...
/**
 * Whether a candidate polygon is geometrically compatible with the computed
 * region and can therefore be considered for in-place restyling.
 *
 ...
 *
 * The computed region's net area and bounds must match the element's. This
 * rejects changed/subdivided regions, but two stacked same-bounds shapes are
 * geometrically indistinguishable; the application layer must additionally
 * verify direct-hit or generated owner/boundary metadata before restyling.
 */
export const isRestylableFill = (args: {
  ...
  const fillArea = polygonAreaExact(ring);
  const regionArea = polygonAreaExact(scenePoints);
  const sameArea =
