...
export function assertProjectStructure(
  ...
  // Projects include legitimate wide collections: slideOrder can reach the
  // total-element ceiling, files may contain up to 10,000 entries, and
  // selected-element maps can track the full per-scene element envelope.
  // Keep the narrower generic limits for other import kinds, but let the
  // explicit project counters below own these compatibility boundaries.
  assertStructuredData(value, {
    maxArrayLength: MAX_PROJECT_TOTAL_ELEMENTS,
    maxObjectKeys: MAX_PROJECT_ELEMENTS_PER_SCENE,
    ...options,
    label,
  });
